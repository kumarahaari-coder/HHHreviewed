...
export default function SettingsAndAudits() {
  ...
  const handleReset = () => {
    if (confirm("Reset this browser's POC data? This clears imported reservations, partners, sites, payouts, and restores the four public HHH stays.")) {
      db.reset();
    }
  };
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="space-y-4 border-rose-200 bg-rose-50/10">
            ...
            <p className="text-xs text-zinc-500 leading-relaxed">
              Clears this browser's local POC snapshot and restores the four Hidden Honey Homes stays sourced from the public website. Live Hospitable reservations must be synced again afterward.
            </p>
            <button
              onClick={handleReset}
            ...
            >
              <RotateCcw size={14} />
              <span>Reset Browser POC Data</span>
            </button>
          </Card>
          ...
          <Card className="space-y-4">
            ...
            <div className="space-y-3 text-xs text-zinc-600">
              <p className="flex items-start gap-1.5">
                ...
                <span>Authentication is still a browser-only POC placeholder; no secure session expiry is active.</span>
              </p>
              <p className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-brand-sage mt-1.5 shrink-0" />
                <span>Supabase Row Level Security is not yet connected and is required before partner access.</span>
              </p>
              <p className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-brand-sage mt-1.5 shrink-0" />
                <span>Guest details are masked in the UI, but database-level partner isolation is still required.</span>
              </p>
            </div>
          </Card>
        </div>
