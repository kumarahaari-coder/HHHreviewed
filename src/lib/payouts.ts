import { Reservation, Partner, Site, CommissionRule, Payout, PayoutStatus } from "./db/schema";
...
export function evaluatePayoutEligibility(res: Reservation): { isEligible: boolean; reason?: string } {
  ...
  if (res.paymentStatus === "UNPAID" || res.paymentStatus === "PARTIAL") {
    return { isEligible: false, reason: "Payment not fully received by HHH." };
  }
...
export function calculatePayout(
  ...
  if (rule.ruleType === "FIXED_PER_BOOKING") {
    ...
    // Dynamic tiering based on booking count this month
    if (partnerBookingsCountThisMonth <= 3) {
      rate = 8; // 8% Gross
    } else if (partnerBookingsCountThisMonth <= 8) {
      rate = 10; // 10% Gross
    } else {
      rate = 12; // 12% Gross
    }
    baseAmount = res.bookingAmount; // default to gross for tiered illustration
    calculatedAmount = baseAmount * (rate / 100);
  }
...
export function runSystemPayoutRecalculation(): void {
  ...
  const updatedReservations = reservations.map(res => {
    ...
    let payout = payouts.find(p => p.reservationId === res.id);
    if (!payout) {
