-- ==============================================================================
-- HHH MIGRATION: 20260803_hhh_map_clerk_user_rpc_role_fix.sql
-- Description: Update map_clerk_user_tx RPC to allow PARTNER_OWNER and admin roles
--              instead of hardcoding expected CREATOR role.
-- ==============================================================================
...
BEGIN
    ...
    -- 3. Role & Lifecycle Assertions (Support all valid application roles)
    ...
    -- For Tenant Roles (PARTNER_OWNER, CREATOR), load and assert partner assignment
    IF v_target_user.role IN ('PARTNER_OWNER', 'CREATOR') THEN
        IF v_target_user.partner_id IS NULL THEN
            RAISE EXCEPTION 'Mapping Aborted: Tenant user % has no assigned partner UUID.', v_target_user.id;
        END IF;
    ...
    -- 5. Perform Idempotent Identity Binding
...
VALUES ('20260803_hhh_map_clerk_user_rpc_role_fix')
